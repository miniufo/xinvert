...
# GPU tests are skipped automatically when a CUDA GPU is not available
archs = ['cpu',
         pytest.param('gpu', marks=pytest.mark.skipif(
             not gpu_available(), reason='CUDA GPU not available'))]


@pytest.mark.parametrize('architect', archs)
def test_poisson_atmos(architect):
...
@pytest.mark.parametrize('architect', archs)
def test_poisson_ocean(architect):
    ds = xr.open_dataset('./Data/Helmholtz_ocean.nc', decode_timedelta=True)
