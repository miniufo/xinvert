...
| Eliassen model\newline[@Eliassen:1952] | $\displaystyle{\frac{\partial}{\partial p}\left(A\frac{\partial \psi}{\partial p}+B\frac{\partial \psi}{\partial y}\right)+\frac{\partial}{\partial y}\left(B\frac{\partial \psi}{\partial p}+C\frac{\partial \psi}{\partial y}\right)=F}$ \newline `sf = invert_Eliassen(F, dims=['Z','Y'], mParams={Angm, Thm})` |
| PV inversion for symmetric \newline vortex [@Hoskins:1985] | $\displaystyle{\frac{\partial}{\partial \theta}\left(\frac{2\Lambda_0}{r^2}\frac{\partial\Lambda}{\partial \theta}\right)+\frac{\partial}{\partial r}\left(\frac{\Gamma g}{Qr}\frac{\partial\Lambda}{\partial r}\right)=0}$ \newline `angM = invert_RefState(PV, dims=['Z','Y'], mParams={ang0, Gamma})` |
| PV inversion QG flow | $\displaystyle{\frac{\partial}{\partial p}\left(\frac{f^2}{N^2}\frac{\partial \psi}{\partial p}\right)+\frac{\partial^2 \psi}{\partial y^2}=q}$ \newline `sf = invert_PV2D(PV, dims=['Z','Y'], mParams={f, N2})` |
| Gill-Matsuno model\newline[@Matsuno:1966; @Gill:1980] | $\displaystyle{A\frac{\partial^2 \phi}{\partial y^2}+B\frac{\partial^2 \phi}{\partial x^2}+C\frac{\partial \phi}{\partial y}+D\frac{\partial \phi}{\partial x}+E\phi=Q}$ \newline `h = invert_GillMatsuno(Q, dims=['Y','X'], mParams={f, epsilon, Phi})` |
| Stommel-Munk model\newline[@Stommel:1948; @Munk:1950] | $\displaystyle{A\nabla^4\psi-\frac{R}{D}\nabla^2\psi-\beta\frac{\partial \psi}{\partial x}=-\frac{\mathbf k\cdot\nabla\times\mathbf{\tau}}{\rho_0 D}}$ \newline `sf = invert_StommelMunk(curl, dims=['Y','X'], mParams={A, R, D, beta, rho})` |
| QG-Omega equation [@Hoskins:1978] | $\displaystyle{\frac{\partial}{\partial p}\left(f^2\frac{\partial \omega}{\partial p}\right)+\nabla\cdot\left(S\nabla\omega\right)=F}$ \newline `w = invert_Omega(F, dims=['Z','Y','X'], mParams={f, S})` |
| 3D ocean flow | $\displaystyle{\frac{\partial}{\partial p}\left(c_3\frac{\partial \phi}{\partial p}\right)+\nabla\cdot\left(c_1\nabla\phi-c_2\hat\nabla\phi\right)=F}$ \newline `w = invert_3DFlow(F, dims=['Z','Y','X'], mParams={f, N2, epsilon})` |
|  | **...** more problems **...** |
